...
func TestNewMemoryStore(t *testing.T) {
	...
	assert.Equal(t, 0, len(store.container), "new store must be empty")
}
...
func TestMemoryStore_Clear(t *testing.T) {
	...
	assert.Equal(t, 0, len(store.container), "store must be empty after Clear")
}
...
func TestMemoryStore_List(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Len(t, entries, 0, "empty store must list zero entries")
...
func TestMemoryStore_KeyTrackingConsistency(t *testing.T) {
	...
	store.mu.RLock()
	assert.Len(t, store.keysList, 0, "index must be empty after Clear")
	store.mu.RUnlock()
}
...
func TestMemoryStore_RandomKey_Distribution_WithTracking(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "", key, "empty store must return empty key")
...
func TestMemoryStore_RandomKey_WithPrefix_TrackingEnabled(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "", key, "empty store must return empty key for any prefix")
	...
	require.NoError(t, err)
	assert.Equal(t, "", key, "no-match prefix must return empty key")
	...
	key, err = store.RandomKey("a:")
	require.NoError(t, err)
	assert.Equal(t, "", key, "after clear, prefix must return empty key")
}
