...
func BenchmarkOSTree_Rank(b *testing.B) {
	...
	require.Greater(b, tree.Len(), 0)
	require.Greater(b, len(queryKeysForRank), 0)
...
func BenchmarkOSTree_Kth(b *testing.B) {
	...
	// Preconditions (outside timed region).
	require.NotNil(b, tree)
	require.Greater(b, totalElementCount, 0)

	b.ReportAllocs()
	b.ResetTimer()

	for range b.N {
		randomIndex := rand.IntN(totalElementCount) //nolint:gosec // benchmark randomness is fine
...
func BenchmarkOSTree_RangeBounds(b *testing.B) {
	...
	require.NotNil(b, tree)
	require.Greater(b, tree.Len(), 0)
	require.Greater(b, len(prefixQueries), 0)
...
func nextAtomicModuloIndex(counter *uint64, limit int) int {
	...
	//nolint:gosec // safe: result < limit (int), so no overflow on the cast.
